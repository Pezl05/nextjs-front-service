...
interface EditTaskProps {
    ...
    task?: Task | null;
    open: boolean;
    onClose: (status: boolean, msg: string) => void;
}
...
export default function ModalEditTask({ isEdit, task, open, onClose }: EditTaskProps) {
    ...
    const [taskDescribe, setTaskDescribe] = useState<string>(task?.description || "");

    const [taskPhase, setTaskPhase] = useState(task?.phase);
    const phaseRef = useRef<HTMLSelectElement | null>(null);

    const [taskStatus, setTaskStatus] = useState(task?.status);
    const statusRef = useRef<HTMLSelectElement | null>(null);

    const [startDate, setStartDate] = useState<string>(task?.start_date?.toString() || "");
    const startDateRef = useRef<HTMLInputElement>(null!);

    const [dueDate, setDueDate] = useState<string>(task?.due_date?.toString() || "");
